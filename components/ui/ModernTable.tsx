      ...
      <table className="w-full min-w-[900px] text-sm">
        ...
        <tbody>
          {customers.map((customer) => (
            <tr key={customer.id} className="border-b border-slate-100 text-slate-600 last:border-0 hover:bg-indigo-50/30">
              ...
              <td className="px-5 py-4 text-indigo-600">{customer.preferredCategory}</td>
              <td className="px-5 py-4">{customer.location}</td>
              <td className="px-5 py-4">
                <div className="h-2 w-24 overflow-hidden rounded-full bg-slate-100">
                  <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-violet-500" style={{ width: `${customer.engagementScore}%` }} />
                </div>
              </td>
            </tr>
          ))}
